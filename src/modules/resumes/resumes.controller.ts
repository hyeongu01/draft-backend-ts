...
export class ResumesController {
  ...
  @ApiOperation({
    ...
    description:
      ...
      '기획상 최초 생성 시에는 빈 값들로 생성되므로(카테고리도 비어 있음), 응답에 category·user 객체는 포함되지 않습니다. category 정보는 수정 후 조회 API 를 통해 확인할 수 있습니다.',
  })
  ...
  async updateItem(
    ...
    const oldItem = await this.resumesService.findItemById(user, id);
    if (!oldItem) throw new NotFoundException('이력서를 찾을 수 없습니다.');
  ...
  @ApiOperation({
    ...
    description:
      ...
      '각 이력서에는 연결된 category 정보(소속 group 포함)가 함께 반환됩니다. 본인 이력서 조회이므로 작성자(user) 정보는 응답에 포함되지 않습니다. 타 유저의 공개 이력서 조회는 별도의 API 를 사용합니다.',
  })
  ...
  @ApiOperation({
    ...
    description:
      '본인이 작성한 이력서 단건을 조회합니다. 연결된 category 정보(소속 group 포함)를 함께 반환하며, 본인 이력서 조회이므로 작성자(user) 정보는 응답에 포함되지 않습니다. ' +
  ...
  async findOne(
    ...
    const item = await this.resumesService.findItemById(user, id);
